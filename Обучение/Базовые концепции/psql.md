...
Запускаем скрипт с дампом в psql

```sql
\l -- смотрим список бд

\c -- переключаемся на необходимю если создана или сначала создаем через CREATE DATABASE base_name;

\i D:/Downloads/sql_append_data.sql -- запускаем скрипт, путь без кавычек, слеши обратные 
```
