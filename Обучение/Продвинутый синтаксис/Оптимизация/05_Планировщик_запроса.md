...
- `Index Scan` - Находит записи в индексе. Все нужные для запроса данные уже есть в самом индексе (все столбцы в SELECT, WHERE, JOIN покрыты индексом). Читать всю таблицу смысла нет, все данные для запроса ест ьв индексе.  
