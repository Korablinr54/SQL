...
### Поиск вхождений
используя оператор `@>` можно проверить вхождение. Собака со стороны массива, в котором осуществляется поиск:  

```sql
SELECT worksheet_jsonb::jsonb @> '{"name": "Колесов Игорь Витальевич"}'::jsonb  
  FROM test.clients;  

-- такой способ проверяет лишь элементы верхнего уровня, т.е. поиск по вложенным структурам, например поиск детей верне false 

SELECT worksheet_jsonb::jsonb @> '{"name": "Колесов Артем"}'::jsonb  
  FROM test.clients;  
```  
Но мы можем сначала получить объект `children` (массив в данном случае) и продолжить поиск в нем:  

```sql
SELECT worksheet_jsonb::jsonb -> 'children' @> '[{"name": "Колесов Артем"}]'::jsonb 
  FROM test.clients;
--вернет true
```